...
# Please submit bugfixes or comments via https://forum.sailfishos.org
#

# If enabled openssl_1_1 provides unversioned /bin/openssl
%bcond_with openssl_1_1_default
...
# Needed for clean upgrade from former openssl
...
%prep
%autosetup -p1 -n %{name}-%{version}/%{_rname}
...
# FIXME: Check if tsget is needed remove it for now
rm %{buildroot}%{_datadir}/ssl/misc/tsget*

%if %{with openssl_1_1_default}
ln -s openssl-1_1 %{buildroot}%{_bindir}/openssl
ln -s openssl-1_1.cnf %{buildroot}%{_sysconfdir}/ssl/openssl.cnf
ln -s c_rehash-1_1 %{buildroot}%{_bindir}/c_rehash
%endif
